...
of its epic, and a change of epic removes it. Ranked siblings stand first, by
rank, and the rest after them by what is happening in them, so a placement, by
a drag on the index or by `session order`, ranks only the worktree placed and
renumbers its siblings only where there is no room between two. Everything else
